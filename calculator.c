...
void processOperator(char character) {
  while (operatorStackTop != -1 &&
         operatorPrecedence(character) <=
             operatorPrecedence(operatorStack[operatorStackTop])) {
    calculate();
  }
...
int isDigit(char character) { return character >= '0' && character <= '9'; }

int isSpace(char character) { return character == ' ' || character == '\t'; }
